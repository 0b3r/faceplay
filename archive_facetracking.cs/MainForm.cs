...
namespace face_tracking.cs
{
    public partial class MainForm : Form
    {
        ...
        private void PopulateProfileMenu()
        {
            ...
            if (session.CreateImpl(ref desc, PXCMFaceAnalysis.CUID, out ft_t) >= pxcmStatus.PXCM_STATUS_NO_ERROR)
            {
                ...
                for (uint i = 0; ; i++)
                {
                    ...
                    if (ftl.QueryProfile(i, out pinfo) < pxcmStatus.PXCM_STATUS_NO_ERROR) break;
        ...
        public uint GetCheckedLandmarkProfile()
        {
            foreach (ToolStripMenuItem m in MainMenu.Items)
            {
                if (!m.Text.Equals("Landmark")) continue;
        ...
        public void DrawLandmark(PXCMFaceAnalysis.Landmark.LandmarkData[] data)
        {
            lock (this)
            {
                if (bitmap == null) return;
